...
def train(args: argparse.Namespace):
    """Solves the FetchReach environment from the OpenAI gym.
    ...
    with open(path, "r") as f:
        config = yaml.load(f, Loader=yaml.SafeLoader)["fetchreach_ddpg"]
    logger.debug("Loading successful, starting experiment")
    n_workers = args.nprocesses
    env = gym.make("FetchReach-v1", reward_type="dense")  # Gym is only created for its constants
    env = FlattenObservation(FilterObservation(env, filter_keys=["observation", "desired_goal"]))
